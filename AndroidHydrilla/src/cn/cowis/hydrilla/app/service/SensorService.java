...
public class SensorService extends Service {
	...
	public class ReadNoCacheParamCycle extends Thread {
		...
		public void run() {
			...
			while (flag) {
				...
				final Object value = getValue(sensorManager,id);
	 ...
	 * 读没有缓存的数据 不循环读只有一次
	 */
	public class ReadNoCacheParamOne extends Thread{
		...
		public ReadNoCacheParamOne(String id,SensorManager sensorManager){
		this.id = id;
		this.sensorManager = sensorManager;
		}
		...
		public void run(){
			...
			final Object value = getValue(sensorManager,id);
				...
				handler.post(new Thread() {
					...
					public void run() {
						backEvents.notifyBackEvent(
								BackEventsType.NO_CACHE_VALUE_ONE, value);
					}
	 ...
	 * @param sensorManager
	 */
	public void startNoCycle(String id,SensorManager sensorManager){
		
		new ReadNoCacheParamOne(id,sensorManager).start();
	}
	...
	private Object getValue(SensorManager sensorManager, String id) {

		Object value = null;
		try {
			if (id.equals(Constants.NO_CACHE_ANALOG)) {
				value = sensorManager.readAnalogResult();
			} else if (id.equals(Constants.NO_CACHE_RESULT1)) {
				
				
			     value = sensorManager.readResult1();
				
			} else if (id.equals(Constants.NO_CACHE_RESULT2)) {
				value = sensorManager.readResult2();
			} else if (id.equals(Constants.NO_CACHE_RESULT3)) {
				value = sensorManager.readResult3();
			} else if (id.equals(Constants.NO_CACHE_RESULT4)) {
